...
static void deep_copy_test()
{
    ...
    std::cout << "\n--- Scope test (shallow copy would crash / double delete) ---\n";
    {
        Dog tmp = basic;
    }
    std::cout << "basic still alive, idea[0]: " << basic.getBrain()->getIdea(0) << "\n";

    std::cout << "\n--- Assignment test ---\n";
    Dog assign;
    assign = basic; // deep copy
